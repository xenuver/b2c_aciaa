...
@section('title', 'Checkout - Beli Langsung')
    ...
    <div class="row">
        <div class="col-md-7">
                            ...
                            <div class="col-12">
                                <div class="address-card p-3 {{ $address->is_default ? 'selected' : '' }}" 
                                    ...
                                    <div class="d-flex align-items-center mb-2">
                                        <span class="badge badge-primary me-2 px-2.5 py-1 rounded">{{ $address->label }}</span>
                                        @if($address->is_default)
                                            <span class="badge bg-success px-2.5 py-1 rounded">Utama</span>
                                        @endif
                                    </div>
                                    <h6 class="fw-bold mb-1">{{ $address->recipient_name }}</h6>
                                    <p class="text-muted small mb-1"><i class="fas fa-phone me-1.5"></i>{{ $address->phone }}</p>
            ...
            <div class="card shadow-sm border-0 mb-4 rounded-4 position-relative d-none" id="serviceSection">
                ...
                <div class="card-body">
                    <div id="serviceList">
                        <!-- Dinonaktifkan atau diisi via AJAX -->
                    </div>
                </div>
        ...
        <div class="col-md-5">
            <div class="card shadow-sm border-0 rounded-4 sticky-top" style="top: 24px;">
                <div class="card-header bg-dark text-white py-3 rounded-top-4">
                    <h5 class="mb-0 fw-bold"><i class="fas fa-clipboard-list me-2"></i>Ringkasan Transaksi</h5>
                </div>
                <div class="card-body">
                    @foreach($cartItems as $item)
                    <div class="d-flex align-items-center mb-3 pb-3 border-bottom">
                        @if($item->product->image)
                            <img src="{{ url('render-image?path=' . $item->product->image) }}" alt="{{ $item->product->name }}" class="rounded me-3" style="width: 60px; height: 60px; object-fit: cover;">
                        @else
                    ...
                    <form action="{{ route('checkout.direct-process') }}" method="POST" id="checkoutForm">
                        @csrf
                        
                        <!-- Hidden inputs yang akan diisi oleh Javascript -->
                        ...
                        <div class="mb-3">
                            ...
                            <textarea name="notes" class="form-control rounded-3" rows="2" placeholder="Contoh: Titipkan di satpam, dll"></textarea>
                        </div>
                    ...
                    <div class="row g-3">
                        ...
                        <div class="col-12">
                            <div class="form-check">
                                ...
                                <label class="form-check-input-label small" for="isDefaultCheck">
...
<script>
    ...
    const productWeight = {{ $cartItems[0]->quantity ?? 1 }} * {{ config('rajaongkir.default_weight', 200) }};
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        // Handle dynamically loading cities on province select change
        ...
        document.getElementById('addAddressForm').addEventListener('submit', function (e) {
            ...
            .then(res => {
                if (res.success) {
                    ...
                    // 3. Re-load address list or append the new address card
    ...
    function selectAddress(element) {
        ...
        // Add selection class to current card
        ...
        // Reset courier and services selections
        resetCourierSection();
        enableCourierSection();
    }
    ...
    function resetCourierSection() {
        ...
        // Hide and clear service section
        const sSection = document.getElementById('serviceSection');
        sSection.classList.add('d-none');
        document.getElementById('serviceList').innerHTML = '';

        // Reset payment summary shipping costs
    ...
    function selectCourier(element) {
        ...
        // Remove selection from all courier buttons
        document.querySelectorAll('.courier-btn').forEach(btn => {
            btn.classList.remove('selected');
        });
    ...
    function fetchShippingCost() {
        ...
        // Show section and show loading spinner
        serviceSection.classList.remove('d-none');
        serviceList.innerHTML = '';
        loading.classList.remove('d-none');

        // Reset submit button and summary values
        document.getElementById('btnSubmitCheckout').disabled = true;
        document.getElementById('summaryShippingCost').textContent = 'Menghitung Ongkir...';
    ...
    function appendAddressCard(address) {
        ...
        col.innerHTML = `
            <div class="address-card p-3" 
                ...
                <div class="d-flex align-items-center mb-2">
                    <span class="badge badge-primary me-2 px-2.5 py-1 rounded">${address.label}</span>
                    ${address.is_default ? '<span class="badge bg-success px-2.5 py-1 rounded">Utama</span>' : ''}
                </div>
                <h6 class="fw-bold mb-1">${address.recipient_name}</h6>
                <p class="text-muted small mb-1"><i class="fas fa-phone me-1.5"></i>${address.phone}</p>
                <p class="small mb-0 text-gray-700">
    ...
    document.getElementById('editAddressForm')?.addEventListener('submit', function(e) {
        ...
        .then(res => {
            ...
            if (card) {
                ...
                card.querySelector('.badge.badge-primary') && (card.querySelector('.badge.badge-primary').textContent = updated.label || 'Alamat');
                ...
                if (phoneP) phoneP.innerHTML = `<i class="fas fa-phone me-1.5"></i>${updated.phone || ''}`;
                ...
                // jika yang diedit adalah alamat terpilih, update city/subdistrict id utk hitung ongkir berikutnya
                if (card.classList.contains('selected')) {
                    selectedAddressId = String(updated.id);
                    selectedCityId = String(updated.city_id);
                    selectedSubdistrictId = updated.subdistrict_id ? String(updated.subdistrict_id) : null;
                }
            }
