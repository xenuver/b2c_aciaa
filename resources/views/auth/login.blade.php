<x-guest-layout>
    ...
    <form method="POST" action="{{ route('login') }}">
        ...
        <div class="d-flex align-items-center justify-content-between mb-4">
            <label for="remember_me" class="d-flex align-items-center" style="cursor:pointer;">
                <input id="remember_me" type="checkbox" name="remember"
                       style="accent-color:#b5838d; width:16px; height:16px; margin-right:8px; border-radius:4px;">
                <span style="font-size:.84rem; color:#666;">Ingat saya</span>
            </label>

            @if (Route::has('password.request'))
                <a href="{{ route('password.request') }}" style="font-size:.84rem; color:#b5838d; text-decoration:none;">
                    Lupa password?
                </a>
            @endif
        </div>
        ...
        <div class="text-center mt-3">
            ...
            <a href="{{ route('register') }}" style="font-size:.84rem; color:#b5838d; text-decoration:none; font-weight:600; margin-left:4px;">Daftar di sini</a>
        </div>
    </form>
</x-guest-layout>
